...
func TestResolveWebTTYFSBaseURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.raw, func(t *testing.T) {
			base, target, err := resolveWebTTYFSBaseURL(tt.raw)
			if err != nil {
...
func TestResolveWebTTYFSBaseURLRejectsRstreamPath(t *testing.T) {
	_, _, err := resolveWebTTYFSBaseURL("rstrm://shell/fs")
	if err == nil {
		t.Fatal("expected rstrm path to be rejected")
	}
}
