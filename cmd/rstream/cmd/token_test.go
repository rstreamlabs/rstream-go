...
func TestCreateTokenRequestFromFlags(t *testing.T) {
	...
	if err := cmd.Flags().Set("resources-json", `{"tunnels":{"projects":{"p1":{"scopes":["connect"]}}}}`); err != nil {
...
func TestTokenCreateResourcesFromFile(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"tunnels":{"projects":{"p1":{"scopes":["connect"]}}}}`), 0o600); err != nil {
