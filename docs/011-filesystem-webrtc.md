...
A prepared context can derive TURN credentials locally without control-plane access when it includes the explicit engine address, project endpoint, TURN domain, realm and ports, and a PAT carrying its token endpoint. Do not select a region that requires runtime discovery on such a device. Incomplete TURN settings or credentials requiring API mode need control-plane access. Credential derivation does not grant permissions beyond the configured token.
